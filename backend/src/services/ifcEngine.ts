...
export class IFCEngine {
  ...
  analyzeIngredients(ingredientsList: string[]): AnalysisResult {
    const normalizedIngredients = ingredientsList.map(i =>
      i.toLowerCase().trim().replace(/[^a-záéíóúñü\s]/g, '')
    );

    const detected: DetectedIngredient[] = [];

    for (const ingredient of this.ingredients) {
      const found = normalizedIngredients.some((ni: string) =>
        ingredient.keywords.some((kw: string) => ni.includes(kw))
      );
  ...
  generateRecommendation(analysis: AnalysisResult, healthGoal: string): string {
  ...
  getScientificReferences(ingredientName: string): string[] {
